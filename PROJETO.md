...
## Migração ETL (admin_lex → LexFirma)

Command `etl:legacy` (`--dry-run`, `--only=Aberto`). Lê da conexão `legacy` (banco `admin_lex`), grava no schema novo. Idempotente (`updateOrCreate`), roda contra `lex_staging` antes de produção.

**Migrado automaticamente:**
- Advogados: Marina Tauro (`active = false`, saiu da sociedade mas consta em processos abertos) e Tula (`active = true`). De-para de nomes do legado no array `$mapAdvogados` (`TULA`, `TULA JUNS`, `MARINA`, `MARINA TAURO`, `MARINA/TULA` → vínculo múltiplo no pivot).
- Clientes: **todos os 267** (decisão: base completa, não perde histórico), juntando `clientes` + `documentos` + `endereco` + `contato` + `conjuge` + `tutelado` nas tabelas relacionadas do novo (`client_documents`, `client_addresses`, `client_contacts`, `spouse`, `wards`).
- Processos: **só os 102 judiciais ativos** (`status = 'Aberto'`, `type = Judicial`). Vínculo cliente por CPF normalizado (mapa em memória `cpfToClientId`); advogados via `$mapAdvogados`.

**NÃO migrado — entrada manual necessária (checklist do primeiro dia):**
- [ ] **9 processos administrativos** (INSS / ADMINISTRATIVO / INVENTÁRIO no campo adverso) — filtrados por `ehNaoJudicial()`. Lançar manualmente como `type = Administrative`. Atenção: alguns "INSS" são cíveis contra o INSS (verificar caso a caso).
- [ ] **Condomínio Edifício Broadway** (CNPJ, cadastrado como PF no legado) — lançar como Enterprise (PJ).
- [ ] **2 CPFs com 10 dígitos** (Jaimara Falcão, Diego Landim) — corrigir dígito faltante.
- [ ] **~18 audiências/agenda** — não migradas (advogada não usa o controle; volume baixo). Lançar manualmente se necessário.
- Tarefas e procurações do legado: não migradas (dado morto / pobre demais — procuração legada só tem tipo+finalidade, sem corpo).

**Tratamentos aplicados no ETL:**
- CPF normalizado (`soNumeros`): remove máscara antes de comparar/gravar.
- Datas-lixo (`0000-00-00`, vazio, `-0001`) → `null` via helper `data()`.
- Relatório de anomalias impresso ao final (CPF inválido, processo sem cliente, sem advogado, administrativos pulados).

## Testes e correções

- Pest 3 instalado. Suíte de feature cobrindo o caminho de notificação de prazos (`DeadlineNotificationTest`, 7 testes) — o fluxo de maior risco do sistema. Factories mínimas: `DeadlineFactory`, `LawyerFactory`, `LegalCaseFactory` (models precisam do trait `HasFactory`).
- Auditoria pós-ETL: validar no staging `legal_cases.client_id` sem nulos, todo processo com ≥1 advogado, contagens (267 clientes / 102 processos / 2 advogados).

## Pendências antes do deploy

- [ ] Rodar ETL definitivo no `lex_staging` e validar auditoria
- [ ] Promover staging → banco de produção na VPS
- [ ] Supervisor rodando worker das filas `default` e `notifications`
- [ ] Cron do HestiaCP apontando para `schedule:run`
- [ ] `redirect_uri` do Google atualizado para o domínio de produção
