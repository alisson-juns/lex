    ...
    protected function getHeaderActions(): array
    {
        return [
                ...
                Action::make('gerarContrato')
                    ...
                    ->form([
                        Forms\Components\Select::make('enterprise_fee_agreement_template_id')
                            ...
                            ->options(
                                EnterpriseFeeAgreementTemplate::where('is_active', true)->pluck('name', 'id')
                            )
                            ->required(),
                        Forms\Components\Select::make('enterprise_representative_id')
                            ->label('Representante Legal')
                    ...
                    ->action(function (array $data, EnterpriseFeeAgreementService $service): void {
                        $agreement = EnterpriseFeeAgreement::create([
                            'enterprise_id'                        => $this->record->id,
            ...
            Action::make('create_case')
                ...
                ->color('gray')
                ->modalHeading(fn () => "Novo processo — {$this->record->corporate_reason}")
                ->modalWidth('3xl')
                ->form([
